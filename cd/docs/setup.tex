...
\begin{figure}[H]
  \centering
  \includegraphics[width=9cm,keepaspectratio]{img/DSC02173.JPG}
  \caption{External power supply connected to AVR Raven} %!TODO foto without JTAG
  \label{psu}
  \bigskip
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=9cm,keepaspectratio]{img/DSC02179.JPG}
  %\caption{USB}
  \label{usb}
  \bigskip
\end{figure}
...
\begin{itemize}
	...
	\item GNU AVR toolchain - avr-libc, binutils-avr, gcc-avr %!TODO - version
	\item Optional debugger (gdb-avr) and interface between GDB and AVR JTAG - avarice - \url{http://avarice.sourceforge.net/}
...
\section{Debugging}
For debugging we will use gdb-avr and avarice software.
We can also use a DDD graphical frontend for debugging.
Avarice tool need a hex file from our project.
If it is not present in the project directory, the following command will extract it:
\begin{lstlisting}
