...
of kilobytes, whereas a typical constrained system has only a few tens of kilobytes of memory~\cite{interconnecting}. %!LYDIA
...
Protosockets are based on Protothreads, which is putting the same limitation on them - such as %LYDIA what imposes
...
It can handle only a single network interface and contains the IP, ICMP, UDP and TCP protocols~\cite{contiki-docs}. %!LYDIA
In order to reduce memory requirements and the code size, %!LYDIA
...
work with data to be transmitted - e.g. if the data is lost in the network, %!LYDIA
the application will be invoked and it has to resend the data. %!LYDIA - time
...
The global packet buffer is large enough to contain one packet of maximum size~\cite{contiki-docs}. %!LYDIA of a

When a packet arrives from the network, the device driver places it in the
global buffer and calls the uIP stack.
If the packet contains data, the uIP stack will notify the corresponding application. %!LYDIA
Because the data in the buffer will be overwritten by the next incoming packet, %!LYDIA
the application will either have to act immediately on the data or copy the data into %!LYDIA act on
its own buffer for later processing.
The packet buffer will not be overwritten by new packets before the application has processed the data~\cite{contiki-docs}. %!LYDIA by
...
that are large enough to contain at least 4 maximum sized Ethernet frames~\cite{contiki-docs}. %!LYDIA
This way, uIP does not have to have its own buffer structures and thus requires only a minimal memory amount. %!LYDIA
Possible packet loss is a trade-off for minimalism and ability to communicate using TCP/IP. %!LYDIA - articles
